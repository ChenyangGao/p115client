...
推荐使用此模式，可以随意登录任何有效的 app。

p115client 也收集了一些有效的 app：

```python
from p115client.const import OPEN_APP_IDS
...
client_open = client.login_another_open(100195125)
```

## 2.授权码模式登录

同一个开放应用 id，最多同时有 3 个登录，如果有新的登录，则自动踢掉较早的那一个。

下面的几个案例代码，展示了用 [授权码模式](https://www.yuque.com/115yun/open/okr2cq0wywelscpe) 授权登录：

### 2.1. 登录 [openlist](https://github.com/OpenListTeam/OpenList)
...
resp = client.login_authorize_open(payload, headers={"cookie": ""})
...
### 2.2. 登录 [clouddrive](https://www.clouddrive2.com)
